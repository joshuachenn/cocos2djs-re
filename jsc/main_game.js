...
// 200676 Jackfruit
t.prototype.onCommunityCards = function(e) {
  this.setRoomCurState(e.roundState);
  this.setPublicCards(e.publicCards, !0, 0);
};

//223522 PSeat
//PSeat: [ function(e, t, n) {
...
//147951 GameMain
t.prototype.OnPlayerShowCards = function(e) {
    for (var t = 0, n = 0; n < e.players.length; ++n) {
      // o=PlayerInfo(326665)/HandRecord(329263)/PlayerShowInfo(338482) i=PSeat
      var o = e.players[n], i = this.getSeatBySeverId(o.seatid);
      if (i) 
        for (var r = 0; r < o.cards.length; ++r) 
          if (!(t >= e.show_card_id.length)) {
            var a = e.show_card_id[t], s = o.cards[r];
            i.getShowCard(a).setContent(s.number, s.suit);
            i.showDown(a + 1);
            o.playerid, l.default.dataHandler.getUserData().u32Uid;
            t += 1;
          }
    }
  };
...
//148022 GameMain
t.prototype.OnCommunityCard = function(e) {
  this.RealCommunityCard(null, e);
  };
  t.prototype.RealCommunityCard = function(e, t) {
  switch (t.betting_round) {
  case l.default.Enum.BettingRoundType.Enum_BettingRound_Preflop:
  break;
  
  case l.default.Enum.BettingRoundType.Enum_BettingRound_Flop:
  for (var n = 0; n < t.cards.length; ++n) {
  var o = t.cards[n];
  this.m_pkPublicCard[n].setContent(o.number, o.suit);
  this.m_pkPublicCard[n].node.active = !0;
  }
  this.DealFlop();
  break;
  
  case l.default.Enum.BettingRoundType.Enum_BettingRound_Turn:
  if (this.m_pkPublicCard[0].node.active && this.m_pkPublicCard[1].node.active && this.m_pkPublicCard[2].node.active) {
  o = t.cards[0];
  this.m_pkPublicCard[3].setContent(o.number, o.suit);
  this.DealTurn();
  }
  break;
  
  case l.default.Enum.BettingRoundType.Enum_BettingRound_River:
  if (this.m_pkPublicCard[0].node.active && this.m_pkPublicCard[1].node.active && this.m_pkPublicCard[2].node.active) {
  o = t.cards[0];
  this.m_pkPublicCard[4].setContent(o.number, o.suit);
  this.DealRiver();
  }
  }
  };
...
this.userData || (this.userData = o.userData.getInstance());
return this.userData;
};

  // 324333
userOptNotify: function(e) {
var t = this, n = arguments.length > 1 && void 0 !== arguments[1] && arguments[1];
HMFUtils.clog("***1用户操作通知（弃牌，加注，跟注):" + JSON.stringify(e));
console.log("***1用户操作通知（弃牌，加注，跟注):" + JSON.stringify(e));
for (var o = 0; o < this._userList.length; o++) {
var i = this._userList[o], r = this.WPGameTableData._seatLayers[i.locialSeatNum];
if (e.userId == i.userId && r.useInfoJs.lblGameStatus.string == HMFUtils.getLanguageStr("WPGame.gameStatus.sit_leave")) return;
}
HMFUtils.clog("***2用户操作通知（弃牌，加注，跟注):" + JSON.stringify(e));
console.log("***2用户操作通知（弃牌，加注，跟注):" + JSON.stringify(e));
for (o = 0; o < this._userList.length; o++) {
i = this._userList[o];
(r = this.WPGameTableData._seatLayers[i.locialSeatNum]).useInfoJs.resetCountdown();
if (e.userId == CurrentUserInfo.user.userId) {
if (e.userId == i.userId) {
(a = this.WPGameTableData._seatLayers[i.locialSeatNum]).userOptInfo(e);
this._gameUI.closeTableSport();
}
} else {
e.userId == i.userId && r.userOptInfo(e, n);
if (i.userId == CurrentUserInfo.user.userId) {
var a = this.WPGameTableData._seatLayers[i.locialSeatNum];
WPGameLocaData.isCardsRecord ? a.seatForPerOpt(this._curRoundMaxBetScore) : this.scheduleOnce(function() {
a.seatForPerOpt(t._curRoundMaxBetScore);
}, .05);
}
}
}
  ...
  function onmessage(e) {
    ...
    if (r.length < 1)
      ...
      // c=serverType l=serverId u=packLen d=msgId p=seq h=playerid
      var serverType = r[0], serverId = r[1], packLen = r[2], msgId = r[3], seq = r[4], playerid = r[5], roomid = r[6];
      cc.log("收包:  u16Msgid:" + msgId + "  U32serverid:" + serverId + "  U32serverType:" + serverType + "  u16PackLen:" + packLen + "  u32seq:" + seq + "  U32playerid:" + playerid + "  U32roomid:" + roomid);
      var _, g = t.getbuffer(), m = this.isEncrypt(serverId);
      if (m) {
        _ = s.aesHandler.DecryptBytes(g);
        console.log(_);
      } else
        console.log(g);
      a.default.roomManager.checkGameIsZoom(serverId) ? serverId = a.default.Enum.GameId.Texas : serverId !== a.default.Enum.GameId.Bet && serverId !== a.default.Enum.GameId.StarSeat && serverId !== a.default.Enum.GameId.Plo || (serverId = a.default.Enum.GameId.Texas);
      var y = this.handlers.get(serverId);
      if (y) {
        var b = y.get(msgId);
        if ("function" == typeof b)
        {
          ...
          a.default.LoadingView.removeWebSocketMsg(serverId, msgId);
        } else console.log("未注册消息id = " + msgId);
      } else console.log("未注册游戏id = " + serverId);
    }
  };
...
//137086 DataView
function readInt8() {
    var e = new DataView(this.buffer);
    this.rpos += 1;
    return e.getInt8(this.rpos - 1);
};
...
function readInt32() {
    var e = this.buffer.slice(this.rpos, this.rpos + 4), t = r.default.ByteBuffer.wrap(e, "utf8", r.default.ByteBuffer.LITTLE_ENDIAN).readInt32();
    this.rpos += 4;
    return t;
};
function readInt64() {
    var e = this.buffer.slice(this.rpos, this.rpos + 8), t = r.default.ByteBuffer.wrap(e, "utf8", r.default.ByteBuffer.LITTLE_ENDIAN).readInt64();
    this.rpos += 8;
    return t;
};
function readUint8() {
    var e = new DataView(this.buffer);
    this.rpos += 1;
    return e.getUint8(this.rpos - 1);
};
function readUint16() {
    var e = new DataView(this.buffer);
    this.rpos += 2;
    return e.getUint16(this.rpos - 2);
};
function readUint32() {
    var e = new DataView(this.buffer);
    this.rpos += 4;
    return e.getUint32(this.rpos - 4);
};
function readUint64() {
    var e = this.buffer.slice(this.rpos, this.rpos + 8), t = r.default.ByteBuffer.wrap(e, "utf8", r.default.ByteBuffer.LITTLE_ENDIAN).readUint64();
    this.rpos += 8;
    return t;
};
function readFloat() {
    try {
        var e = new Float32Array(this.buffer, this.rpos, 1);
    } catch (t) {
        e = new Float32Array(this.buffer.slice(this.rpos, this.rpos + 4));
    }
    this.rpos += 4;
    return e[0];
};
function readDouble() {
    try {
        var e = new Float64Array(this.buffer, this.rpos, 1);
    } catch (t) {
        e = new Float64Array(this.buffer.slice(this.rpos, this.rpos + 8), 0, 1);
    }
    this.rpos += 8;
    return e[0];
};
function readString() {
    for (var e = new Uint8Array(this.buffer, this.rpos), t = 0, n = ""; ;) {
        if (0 == e[t]) {
            t++;
            break;
        }
        ...
        t++;
        if (this.rpos + t >= this.buffer.byteLength) 
            throw new Error("KBEngine.MemoryStream::readString: rpos(" + (this.rpos + t) + ")>=" + this.buffer.byteLength + " overflow!");
    }
    this.rpos += t;
    return n;
};
